        ...
        [
            tag('div', class: 'relative left-0 right-0 bottom-0 top-0 w-full', children:
                $this->subscription->course->cover_image_media_id->unwrapOr(false) ?
                    scTag('img', class: 'absolute w-auto h-auto left-0 right-0 top-0 bottom-0', src: URLGenerator::generateFileUrl($this->subscription->course->coverMedia->fileNameFromBaseDir()))
                :
                    text('Sem imagem!')
            ),
